...
export const PAGE_HELP_CONFIG: Record<string, PageHelpConfig> = {
  ...
  "/game-start": {
    ...
    content: [
      ...
      "1. Make sure your scout name is selected in the sidebar first",
  ...
  "/pit-assignments": {
    ...
    content: [
      ...
      "**Getting Started:**",
      "1. **Add Scouts** - Use the Scout Management section to add team members who will conduct pit scouting",
      ...
      "",
      "**Assignment Modes:**",
      "• **Block Assignment** - Automatically divides teams into consecutive blocks for each scout",
      "  - Teams are sorted numerically and distributed evenly",
      "  - Each scout gets a sequential block of teams (e.g., Teams 1-10, 11-20, etc.)",
      "  - Perfect for systematic coverage with minimal overlap at events with sequential pit locations",
      "",
      "• **Manual Assignment** - Click-to-assign individual teams to specific scouts",
      "  - Select a scout from the colored buttons, then click team cards to assign",
      "  - Right-click team cards to remove assignments",
      "  - Must confirm assignments before scouts can mark teams as completed",
      "",
      "**Two View Modes:**",
      "• **Team Cards** - Visual grid layout with color-coded assignments",
      "  - Each team shows as a card with scout color coding",
      ...
      "  - Sort by team number, assigned scout, or completion status",
      "  - Search for specific teams or scouts",
      "  - Export assignments to CSV for external sharing",
      "",
      "**Progress Tracking:**",
      "• Visual progress bars show completion percentage",
      "• Color-coded scout legend with assignment counts",
      ...
      "- Manual assignments are ideal when you need specific scout-team pairings",
  ...
  "/peer-transfer": {
    ...
    useDialog: true,
    content: [
      "Fast peer--to-peer data transfer over WiFi or cellular networks using room codes. Connections persist across navigation and auto-reconnect on disconnect. Requires all devices to have internet connectivity. For offline environments, use the QR Code transfer method instead.",
      "",
      "**Data Transfer Types:**",
      ...
      "• **Scout Profiles** - Team member profiles",
      "• **Combined Data** - Scouting data + scout profiles in one transfer",
      "",
      "**Lead Scout Features:**",
      "• **Individual Actions** - Request or push data to specific scouts",
      ...
      "• **Navigate Away** - Connections stay active when you leave the page",
      "• **Auto-Reconnect** - Scouts rejoin automatically when you return",
      "",
      "**Scout Features:**",
      "• **Simple Join** - Enter room code once, stay connected",
      "• **Approve/Decline** - Control what data you send or receive",
      "• **Background Connection** - Use other app features while staying connected",
      "• **Visual Indicators** - Connection status shown clearly",
      "• **Auto-Reconnect** - Reconnects automatically if disconnected",
      "",
      "**Connection Status:**",
      "• **Connected** ✅ - Active WebRTC connection, ready for data transfer",
      "• **Connecting** 🔄 - Establishing connection via signaling server",
      "• **Disconnected** ⚠️ - Connection lost, attempting auto-reconnect",
      "• **Timeout** ⏱️ - 30-second connection timeout with countdown indicator",
      "",
      "**Transfer History:**",
      "• **Recent Transfers** - View all completed data exchanges",
      "• **Clear History** - Trash button to remove all transfer records",
      "• **Transfer Details** - See data type, scout name, timestamp, and record count",
      "",
      "**Troubleshooting:**",
      "- **Connection Timeout?** Verify all devices have internet access (WiFi or cellular)",
      "- **Scout Not Connecting?** They may need to refresh and re-enter the code",
      "- **Auto-Reconnect Failed?** Try clearing saved room code and generating new one",
      "- **Lead Navigation Issues?** Wait a few seconds after returning for scouts to announce",
      "- **Multiple Scouts Disconnecting?** Check lead's internet connection stability",
      "- **No Internet Available?** Fall back to QR Code transfer method for offline use",
      "",
      "**Best Practices:**",
      "• Share room code clearly - verbal, text message, or screen display",
      "• Keep lead device stable with good internet connection",
      "• Scouts can navigate freely - connections persist globally",
      "• Generate new code for each event or session",
      "• Use 'Push to All' for distributing schedules efficiently",
      "• Monitor scout cards for connection status",
      "• Clear saved room code when starting fresh",
      "",

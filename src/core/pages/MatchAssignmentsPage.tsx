...
const MatchAssignmentsPage: React.FC = () => {
  ...
  const { selectedEvent, teamDataSource, teamCount } = usePreferredEventTeams();
  ...
  const [scheduleRows, setScheduleRows] = useState(() =>
    normalizeStoredMatchSchedule(JSON.parse(localStorage.getItem('matchData') || '[]')),
  );
  ...
  const hasRequiredData = hasData && scheduleRows.length > 0 && availableScouts.length > 0;
        ...
        <Alert>
          ...
          <AlertDescription>
            {scheduleRows.length === 0 &&
              'No match schedule found. Import schedule data from the TBA Data page first.'}
            {availableScouts.length === 0 &&
              ' Add scouts or connect scout devices over WebRTC before assigning stations.'}
          </AlertDescription>
        </Alert>
      )}
    </div>
